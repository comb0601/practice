/* Lesson 53, Example 07: IP address control
 * Advanced WinAPI program demonstrating IPAddress
 */
#define UNICODE
#define _UNICODE
#include <windows.h>
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int nShow) {
    WNDCLASSEX wc = {sizeof(wc), CS_HREDRAW|CS_VREDRAW, WindowProc, 0, 0,
                     ...
                     0, L"IPAddressClass", 0};
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindowEx(0, L"IPAddressClass", L"Lesson 53: IP address control",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_PAINT: {
            ...
            wchar_t title[256];
            wsprintf(title, L"Lesson 53: IPAddress");
            TextOut(hdc, 10, 10, title, (int)wcslen(title));
            
            const wchar_t* desc = L"IP address control";
