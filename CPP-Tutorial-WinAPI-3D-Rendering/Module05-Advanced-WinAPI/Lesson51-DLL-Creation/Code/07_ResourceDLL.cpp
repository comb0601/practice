/* Lesson 51, Example 07: Resource-only DLL
 * Advanced WinAPI program demonstrating ResourceDLL
 */
#define UNICODE
#define _UNICODE
#include <windows.h>
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int nShow) {
    WNDCLASSEX wc = {sizeof(wc), CS_HREDRAW|CS_VREDRAW, WindowProc, 0, 0,
                     ...
                     0, L"ResourceDLLClass", 0};
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindowEx(0, L"ResourceDLLClass", L"Lesson 51: Resource-only DLL",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_PAINT: {
            ...
            wchar_t title[256];
            wsprintf(title, L"Lesson 51: ResourceDLL");
            TextOut(hdc, 10, 10, title, (int)wcslen(title));
            
            const wchar_t* desc = L"Resource-only DLL";
