/* Lesson 51, Example 02: __declspec(dllexport)
 * Advanced WinAPI program demonstrating ExportFunctions
 */
#define UNICODE
#define _UNICODE
#include <windows.h>
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int nShow) {
    WNDCLASSEX wc = {sizeof(wc), CS_HREDRAW|CS_VREDRAW, WindowProc, 0, 0,
                     ...
                     0, L"ExportFunctionsClass", 0};
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindowEx(0, L"ExportFunctionsClass", L"Lesson 51: __declspec(dllexport)",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE:
            // Initialize: __declspec(dllexport)
            return 0;
        ...
        case WM_PAINT: {
            ...
            wchar_t title[256];
            wsprintf(title, L"Lesson 51: ExportFunctions");
            TextOut(hdc, 10, 10, title, (int)wcslen(title));
            
            const wchar_t* desc = L"__declspec(dllexport)";
