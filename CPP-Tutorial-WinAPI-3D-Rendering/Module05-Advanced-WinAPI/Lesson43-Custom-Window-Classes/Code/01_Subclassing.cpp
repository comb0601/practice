/* Lesson 43, Example 01: Subclassing existing windows
 * Advanced WinAPI program demonstrating Subclassing
 */
#define UNICODE
#define _UNICODE
#include <windows.h>
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int nShow) {
    WNDCLASSEX wc = {sizeof(wc), CS_HREDRAW|CS_VREDRAW, WindowProc, 0, 0,
                     ...
                     0, L"SubclassingClass", 0};
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindowEx(0, L"SubclassingClass", L"Lesson 43: Subclassing existing windows",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE:
            // Initialize: Subclassing existing windows
            return 0;
        ...
        case WM_PAINT: {
            ...
            wchar_t title[256];
            wsprintf(title, L"Lesson 43: Subclassing");
            TextOut(hdc, 10, 10, title, (int)wcslen(title));
            
            const wchar_t* desc = L"Subclassing existing windows";
