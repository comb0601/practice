/* Lesson 43, Example 08: Control creation and destruction
 * Advanced WinAPI program demonstrating ControlLifecycle
 */
#define UNICODE
#define _UNICODE
#include <windows.h>
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int nShow) {
    WNDCLASSEX wc = {sizeof(wc), CS_HREDRAW|CS_VREDRAW, WindowProc, 0, 0,
                     ...
                     0, L"ControlLifecycleClass", 0};
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindowEx(0, L"ControlLifecycleClass", L"Lesson 43: Control creation and destruction",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE:
            // Initialize: Control creation and destruction
            return 0;
        ...
        case WM_PAINT: {
            ...
            wchar_t title[256];
            wsprintf(title, L"Lesson 43: ControlLifecycle");
            TextOut(hdc, 10, 10, title, (int)wcslen(title));
            
            const wchar_t* desc = L"Control creation and destruction";
