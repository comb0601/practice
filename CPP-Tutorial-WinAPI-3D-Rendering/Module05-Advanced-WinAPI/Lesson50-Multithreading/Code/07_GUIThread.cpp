/* Lesson 50, Example 07: Updating GUI from thread
 * Advanced WinAPI program demonstrating GUIThread
 */
#define UNICODE
#define _UNICODE
#include <windows.h>
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int nShow) {
    WNDCLASSEX wc = {sizeof(wc), CS_HREDRAW|CS_VREDRAW, WindowProc, 0, 0,
                     ...
                     0, L"GUIThreadClass", 0};
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindowEx(0, L"GUIThreadClass", L"Lesson 50: Updating GUI from thread",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE:
            // Initialize: Updating GUI from thread
            return 0;
        ...
        case WM_PAINT: {
            ...
            wchar_t title[256];
            wsprintf(title, L"Lesson 50: GUIThread");
            TextOut(hdc, 10, 10, title, (int)wcslen(title));
            
            const wchar_t* desc = L"Updating GUI from thread";
