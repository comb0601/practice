/* Lesson 50, Example 05: Mutex objects
 * Advanced WinAPI program demonstrating Mutex
 */
#define UNICODE
#define _UNICODE
#include <windows.h>
...
int WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, PWSTR, int nShow) {
    WNDCLASSEX wc = {sizeof(wc), CS_HREDRAW|CS_VREDRAW, WindowProc, 0, 0,
                     ...
                     0, L"MutexClass", 0};
    RegisterClassEx(&wc);
    HWND hwnd = CreateWindowEx(0, L"MutexClass", L"Lesson 50: Mutex objects",
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_PAINT: {
            ...
            wchar_t title[256];
            wsprintf(title, L"Lesson 50: Mutex");
            TextOut(hdc, 10, 10, title, (int)wcslen(title));
            
            const wchar_t* desc = L"Mutex objects";
